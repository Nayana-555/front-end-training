      ...
      return `
        <div class='item' >
            ...
            <h1>Question ${item.id}</h1>
              <h3>  <strong>Q. ${item.question}</strong></h3>
                <div class='options'>
                    
                    <input type='radio' name='${item.id}'value='option1'> ${item.option1}<br>
                    <input type='radio' name='${item.id}'value='option2'> ${item.option2}<br>
                    <input type='radio' name='${item.id}'value='option3'> ${item.option3}<br>
...
const fetchQuestions = async () => {
  const response = await fetch(
    "https://6083c8329b2bed0017040391.mockapi.io/api/questions"
  );
...
const fetchCrctAns = async (userAnswers) => {
  const response = await fetch(
    "https://6083c8329b2bed0017040391.mockapi.io/api/answers"
  );
